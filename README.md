...
### Заголовки при запросе подписки

При запросе subscription отправляются следующие заголовки (совместимость с xray-checker):
...
```env
# .env
HWID=your-unique-hardware-id
```

Если HWID не задан, используется дефолтное значение `youtube-monitor-default-hwid`.
